...
class DataHandler:
    ...
    def load_environ_or_config_settings(self):
        ...
        try:
            ...
            if os.path.exists(self.settings_config_file):
                self.diagnostic_logger.info("Loading Config via file")
        ...
        if self.recommended_books:
            if self.clients_connected_counter == 0:
                if len(self.recommended_books) > 25:
                    ...
                    self.diagnostic_logger.info("Shuffling Books")
    ...
    def request_books_from_readarr(self, checked=False):
        try:
            self.diagnostic_logger.info("Getting Books from Readarr")
            ...
            response_authors = requests.get(
                endpoint_authors,
                headers=headers,
                timeout=self.readarr_api_timeout,
            )
    ...
    def find_similar_books(self):
        if self.stop_event.is_set() or self.search_in_progress_flag:
            if self.search_in_progress_flag:
                self.diagnostic_logger.info("Searching already in progress")
                socketio.emit(
                    "new_toast_msg",
                    {"title": "Search in progress", "message": "It's just slow...."},
                )
            return
        elif not self.search_exhausted_flag:
            try:
                self.diagnostic_logger.info("Searching for new books")
                socketio.emit(
                    "new_toast_msg",
                    {
                ...
                self.search_in_progress_flag = False
                self.diagnostic_logger.info("Finished Searching")
...
def load_more_books():
    thread = threading.Thread(
        target=data_handler.find_similar_books,
        name="Find_Similar",
    )
    thread.daemon = True
    thread.start()
