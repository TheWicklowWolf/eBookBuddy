        ...
        try:
            ...
            try:
                ...
                self.diagnostic_logger.info(
                    self.diagnostic_logger.info("Waiting to see if Overlay is displayed...")
                )
                overlay = wait.until(
            ...
            try:
                ...
                if overlay:
                    self.diagnostic_logger.info(
                        f"Overlay displayed on search, attempting to close it..."
                    )
            ...
            try:
                ...
                try:
                    wait = WebDriverWait(driver, self.goodreads_wait_delay)
                    self.diagnostic_logger.info(
                        f"Waiting to see if Overlay is displayed..."
                    )
                    overlay = wait.until(
                        lambda driver: self.stop_event.is_set()
                        or EC.visibility_of_element_located(
                ...
                if overlay:
                    try:
                        self.diagnostic_logger.info(
                            f"Overlay displayed on book link, attempting to close it..."
                        )
                        overlay.click()
                ...
                try:
                    ...
                    wait = WebDriverWait(driver, self.goodreads_wait_delay)
                    self.diagnostic_logger.info(
                        f"Waiting until Carousel is displayed..."
                    )
                    carousel = wait.until(
                        lambda driver: self.stop_event.is_set()
                        or EC.visibility_of_element_located(
                    ...
                    try:
                        self.diagnostic_logger.info(
                            f"Could not find Carousel on first attempt trying again..."
                        )
                        wait = WebDriverWait(driver, self.goodreads_wait_delay)
                        self.diagnostic_logger.info(
                            f"Waiting until Carousel is displayed..."
